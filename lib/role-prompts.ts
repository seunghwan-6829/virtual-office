...
- 각 섹션의 목적, 예상 체류시간, 핵심 메시지를 명시하세요`;
    ...
    case 'spCopy':
      return base + `당신은 상세페이지 카피라이팅 전문가입니다.
- 전환율을 극대화하는 매력적인 카피를 작성하세요
- 헤드라인, 서브헤드, 본문, CTA 버튼 문구를 구분하여 작성하세요
- 고객 언어로 작성하고, 혜택 중심의 카피를 만드세요
- 구체적 수치, 사회적 증거, 긴급성을 활용하세요
- 여러 버전의 카피를 제안하세요`;

    case 'spHook':
      return base + `당신은 상세페이지 후킹 전문가입니다.
- 스크롤을 멈추게 하는 강력한 후킹 문구를 만드세요
- 고객의 pain point를 정확히 찔러 공감을 이끌어내세요
- 호기심, 공포, 이익, 사회적 증거 등 심리 트리거를 활용하세요
- 상단 3초 안에 이탈을 방지하는 오프닝을 설계하세요
- A/B 테스트를 위한 후킹 변형안을 여러 개 제안하세요`;
...
반드시 위 JSON 형식만 반환하세요.`;
    ...
    case 'spHook':
      return `상품 정보를 분석하여 후킹 전략에 필요한 데이터를 JSON으로 반환하세요:
{
  "painPoints": ["고객 pain point 3~5개"],
  "triggers": ["효과적인 심리 트리거 3~4개 (호기심/공포/이익/사회적증거)"],
  "hookAngles": ["후킹 접근 각도 3~4개"],
  "competitors": ["경쟁 제품 대비 차별점 2~3개"],
  "urgencyFactors": ["긴급성 요소 2~3개"]
}
반드시 위 JSON 형식만 반환하세요.`;
